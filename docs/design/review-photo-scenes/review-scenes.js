...
import { GroundedSkybox } from 'https://esm.sh/three@0.170.0/examples/jsm/objects/GroundedSkybox.js';
...
// Public assets, all CC0 from Poly Haven. Fetched through its API by default; the game should
// ship copies and pass setAssetResolver a function returning local URLs for the same ids.
export const ASSETS = {
...
let resolver = async (id, kind, res) => {
  const f = await (await fetch('https://api.polyhaven.com/files/' + id)).json();
  if (kind === 'hdri') return { url: f.hdri[res].hdr.url };
  if (kind === 'texture') return { maps: { map: f.Diffuse[res].jpg.url, rough: f.Rough[res].jpg.url, nor: f.nor_gl[res].jpg.url } };
  const g = f.gltf[res].gltf;
  return { url: g.url, include: Object.fromEntries(Object.entries(g.include).map(([k, v]) => [k, v.url])) };
};
export function setAssetResolver(fn) { resolver = fn; }
const cache = new Map();
...
export function loadHdri(id, res = '2k') {
  return once('h' + id + res, async () => {
    ...
    const t = await new RGBELoader().setDataType(THREE.FloatType).loadAsync(r.url);
...
function woodMat() {
  return once('woodMat', async () => {
    try {
      ...
      return new THREE.MeshStandardMaterial({ map, roughnessMap: rough, normalMap: nor, roughness: 1 });
    } catch (e) { return null; }
  });
}
