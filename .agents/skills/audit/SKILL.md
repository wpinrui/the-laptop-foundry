...
Run `node .codex/scripts/audit.mjs` (append `--all` only when requested) from the repository root, then read its output.

Summarise the command output: what breached a cap, what is uncalled, and what
