...
`.agents/skills/review/SKILL.md`. That bar is the authority; this file says how to
...
Read files and search using the available tools. Shell reads such as `rg` and PowerShell `Get-Content` are allowed. Use the repository root as the working directory. Every dimension is read-only except explicitly requested DEEP MODE mutation and restoration.
